...
boru-driven command-line client **and** interactive REPL for the Disney
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/disney-cli)
make build
...
./disney-cli list character
./disney-cli load 1 character            # {id:1} shorthand
./disney-cli load '{id:1}' character       # explicit match map

# 5. Override the API base URL for a single call
DISNEY_BASE=https://api.example.com ./disney-cli list character

# 6. No arguments -> interactive REPL
./disney-cli
disney> list character
disney> /quit
```
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/disney-cli
   ```
...
3. **Run a query.** Evaluate an boru expression against the API (or run with no
   ...
   ```sh
   ./dist/*/disney-cli list character
   ```
...
```sh
./disney-cli load 1 character          # scalar shorthand for {id:1}
./disney-cli load '{id:1}' character     # explicit match map
```
...
Run with no arguments to open a REPL (prompt `disney>`). Each line is
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/disney-cli`. |
...
(`./disney-cli <expr>`) and interactively (the REPL), and expressions compose the
