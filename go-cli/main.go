// Command Disney-cli is a CLI + REPL for the Disney SDK,
...
func usage(out io.Writer) {
	...
	fmt.Fprintln(out, "Env:      DISNEY_APIKEY (api key), DISNEY_BASE (base url override)")
...
func run(args []string, in io.Reader, out, errOut io.Writer) int {
	...
	// Configure from the environment: DISNEY_APIKEY carries the API key and
	// DISNEY_BASE optionally overrides the API base URL (e.g. production).
	// Both injectable by a secrets vault. Unset -> nil config defaults.
	var opts map[string]any
	if apikey := os.Getenv("DISNEY_APIKEY"); apikey != "" {
		opts = map[string]any{"apikey": apikey}
	}
	if base := os.Getenv("DISNEY_BASE"); base != "" {
		if opts == nil {
			opts = map[string]any{}
		}
		opts["base"] = base
	}
	client := sdk.NewDisneySDK(opts)
