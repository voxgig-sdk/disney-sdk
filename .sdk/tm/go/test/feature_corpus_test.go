...
//
// NewDisneySDK, not TestSDK: the `test` feature is transport: 'base' and
// REPLACES the transport, so a client in test mode would shadow the script.
func fcClient(kase map[string]any) *sdk.DisneySDK {
	res, _ := kase["res"].([]any)
	opts := map[string]any{
...
func fcCandidates(client *sdk.DisneySDK) []fcOp {
	out := []fcOp{}
...
func fcInvoke(client *sdk.DisneySDK, op fcOp, ctrl map[string]any) error {
	cv := reflect.ValueOf(client)
	ct := cv.Type()
	mapType := reflect.TypeOf(map[string]any{})
...
// new go here.
func fcRecord(client *sdk.DisneySDK, name string) any {
