...
The Ruby SDK for the Disney API — an entity-oriented client using idiomatic Ruby conventions.
...
```ruby
client = DisneySDK.test(testopts, sdkopts)
```
...
raise a `DisneyError` (a `StandardError` subclass), so wrap
...
The main module (`Disney_sdk`) exports the SDK class
