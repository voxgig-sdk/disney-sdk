...
//     (\`new DisneySDK(...)\` / \`DisneySDK.test(...)\` become
//     \`DisneySDK.test({ entity: ... })\`), transpiled to JS, and run
...
const TEST_SEED = {"entity":{"character":{"test01":{"id":"test01"}}}}
...
// rewrite real client construction (\`new DisneySDK(...)\` and any
// \`DisneySDK.test(...)\`) to a seeded test-mode client, and give
...
function rewriteForRun(code: string): string {
  ...
  out = out.replace(/new\s+DisneySDK\s*\([^)]*\)/g, () => SEEDED_CTOR)
  out = out.replace(/DisneySDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR)
