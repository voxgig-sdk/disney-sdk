...
import { DisneySDK, BaseFeature, stdutil } from '../../..'
...
describe('CharacterEntity', async () => {
  ...
  // `test.live.delayMs`; only sleeps when DISNEY_TEST_LIVE=TRUE.
  afterEach(liveDelay('DISNEY_TEST_LIVE'))

  test('instance', async () => {
    const testsdk = DisneySDK.test()
  ...
  test('basic', async (t) => {
    ...
    const live = 'TRUE' === process.env.DISNEY_TEST_LIVE
    ...
    if (setup.syntheticOnly) {
      t.skip('live entity test uses synthetic IDs from fixture — set DISNEY_TEST_CHARACTER_ENTID JSON to run live')
      return
    }
...
function basicSetup(extra?: any) {
  ...
  let client = DisneySDK.test(options, extra)
  ...
  const idmapEnvVal = process.env['DISNEY_TEST_CHARACTER_ENTID']
  ...
  const setup = {
    ...
    struct,
    data: entityData,
    explain: 'TRUE' === env.DISNEY_TEST_EXPLAIN,
    live,
