...
class CharacterEntityTest extends TestCase
{
    ...
    public function test_stream(): void
    {
        ...
        $base = DisneySDK::test($seed, null);
        $seen = iterator_to_array($base->Character(null)->stream("list", null, null), false);
        $this->assertCount(3, $seen);

        // Inbound: streaming active -> yields each item from the feature.
        $cfg = DisneyConfig::shared_config();
        if (isset($cfg["feature"]) && is_array($cfg["feature"]) && isset($cfg["feature"]["streaming"])) {
            $sdk = DisneySDK::test($seed, ["feature" => ["streaming" => ["active" => true]]]);
            $got = [];
    ...
    public function test_basic_flow(): void
    {
        ...
        if (!empty($setup["synthetic_only"])) {
            $this->markTestSkipped("live entity test uses synthetic IDs from fixture — set DISNEY_TEST_CHARACTER_ENTID JSON to run live");
            return;
        }
        $client = $setup["client"];
...
function character_basic_setup($extra)
{
    ...
    $client = DisneySDK::test($options, $extra);
    ...
    $entid_env_raw = getenv("DISNEY_TEST_CHARACTER_ENTID");
    ...
    $idmap_resolved = Helpers::to_map(
        $env["DISNEY_TEST_CHARACTER_ENTID"]);
    ...
    if ($env["DISNEY_TEST_LIVE"] === "TRUE") {
        ...
        $client = new DisneySDK(Helpers::to_map($merged_opts));
    }
    ...
    return [
        ...
        "env" => $env,
        "explain" => $env["DISNEY_TEST_EXPLAIN"] === "TRUE",
        "live" => $live,
