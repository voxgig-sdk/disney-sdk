...
class FtCtrl extends DisneyControl
{
...
class FtClient
{
    ...
    private DisneyUtility $utility;

    public function __construct(DisneyUtility $utility, array $options)
    {
    ...
    public function get_utility(): DisneyUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (DisneyContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public function __construct(
        ...
        $utility->fetcher = $server ?? function (DisneyContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = DisneyConfig::make_feature($name);
    ...
    private static function buildUrl(DisneySpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof DisneyError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new DisneyResponse($fetched);
        }
    ...
    private function fail(DisneyContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (DisneyContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
