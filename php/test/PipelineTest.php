...
{
    private static function utility(): DisneyUtility
    {
        return new DisneyUtility();
    }
    ...
    private static function ctx(array $over = []): DisneyContext
    {
        $utility = $over['utility'] ?? self::utility();
        $client = $over['client'] ?? new PlClient(['base' => 'http://h']);
        $ctx = new DisneyContext([
            'client' => $client,
            'utility' => $utility,
        ], null);
        $ctx->op = new DisneyOperation(['name' => 'load', 'entity' => 'x']);
        foreach ($over as $k => $v) {
    ...
    {
        return ($err instanceof DisneyError) ? $err->sdk_code : '';
    }
    ...
    public function test_make_point_rejects_a_disallowed_operation(): void
    {
        ...
        $ctx->op = new DisneyOperation(['name' => 'nope', 'entity' => 'x']);
        [$point, $err] = DisneyMakePoint::call($ctx);
    ...
    public function test_make_point_rejects_an_operation_with_no_endpoints(): void
    {
        ...
        $ctx->op = new DisneyOperation(['name' => 'load', 'entity' => 'x', 'points' => []]);
        [$point, $err] = DisneyMakePoint::call($ctx);
        $this->assertNull($point);
    ...
    public function test_make_point_returns_the_single_point(): void
    {
        ...
        $ctx = self::ctx(['options' => ['allow' => ['op' => 'load,list,create,update,remove']]]);
        $ctx->op = new DisneyOperation(['name' => 'load', 'entity' => 'x', 'points' => [$point]]);
        [$got, $err] = DisneyMakePoint::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new DisneySpec(['method' => 'GET']);
        $ctx = self::ctx();
        $ctx->out['spec'] = $preset;
        [$got, $err] = DisneyMakeSpec::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    public function test_make_response_guards_missing_spec_response_result(): void
    {
        $ctx = self::ctx([
            'spec' => null,
            'response' => new DisneyResponse([]),
            'result' => new DisneyResult([]),
        ]);
        [, $err] = DisneyMakeResponse::call($ctx);
    ...
    {
        $ctx = self::ctx([
            'spec' => new DisneySpec(['step' => 's']),
            'response' => new DisneyResponse(self::resp(404, null, ['x-a' => '1'])),
            'result' => new DisneyResult([]),
        ]);
        [, $err] = DisneyMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_2xx_parses_the_body_and_marks_ok(): void
    {
        $ctx = self::ctx([
            'spec' => new DisneySpec(['step' => 's']),
            'response' => new DisneyResponse(self::resp(200, ['v' => 1])),
            'result' => new DisneyResult([]),
        ]);
        [, $err] = DisneyMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_records_to_ctrl_explain_when_explain_is_on(): void
    {
        $ctx = self::ctx([
            'spec' => new DisneySpec(['step' => 's']),
            'response' => new DisneyResponse(self::resp(200, ['v' => 2])),
            'result' => new DisneyResult([]),
        ]);
        $ctx->ctrl->explain = ['on' => true];
        DisneyMakeResponse::call($ctx);
    ...
    {
        $preset = new DisneyResponse(self::resp(299));
        $ctx = self::ctx([
            'spec' => new DisneySpec([]),
            'response' => new DisneyResponse([]),
            'result' => new DisneyResult([]),
        ...
        [$got, $err] = DisneyMakeResponse::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null, 'result' => new DisneyResult([])]);
        [, $err] = DisneyMakeResult::call($ctx);
        $this->assertSame('result_no_spec', self::code($err));

        $ctx = self::ctx(['spec' => new DisneySpec([]), 'result' => null]);
        [, $err] = DisneyMakeResult::call($ctx);
    ...
    public function test_make_result_list_op_wraps_resdata_into_entity_instances(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new DisneySpec(['step' => 's']),
            'result' => new DisneyResult(['ok' => true, 'resdata' => [['a' => 1], ['a' => 2]]]),
        ]);
        $ctx->op = new DisneyOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = DisneyMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_result_empty_list_yields_empty_resdata(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new DisneySpec(['step' => 's']),
            'result' => new DisneyResult(['ok' => true, 'resdata' => []]),
        ]);
        $ctx->op = new DisneyOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = DisneyMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new DisneyResult(['ok' => true]);
        $ctx = self::ctx([
            'spec' => new DisneySpec([]),
            'result' => new DisneyResult([]),
        ]);
        $ctx->out['result'] = $preset;
        [$got, $err] = DisneyMakeResult::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    public function test_make_request_guards_a_missing_spec(): void
    {
        ...
        [, $err] = DisneyMakeRequest::call($ctx);
    ...
    {
        $utility = self::utility();
        $boom = new DisneyError('boom', 'boom');
        $utility->fetcher = function (DisneyContext $_c, string $_u, array $_f) use ($boom): array {
            return [null, $boom];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new DisneySpec(['step' => 's', 'method' => 'GET', 'base' => 'http://h', 'parts' => ['a']]),
        ]);
        [$response, $err] = DisneyMakeRequest::call($ctx);
        $this->assertNull($err);
    ...
    {
        $utility = self::utility();
        $utility->fetcher = function (DisneyContext $_c, string $_u, array $_f): array {
            return [null, null];
        };
        $ctx = self::ctx([
            'utility' => $utility,
    ...
    public function test_make_request_a_normal_transport_response_is_wrapped(): void
    {
        ...
        [$response, $err] = DisneyMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertInstanceOf(DisneyResponse::class, $response);
    ...
    {
        $utility = self::utility();
        $utility->make_fetch_def = function (DisneyContext $c): array {
            return [null, $c->make_error('fetchdef_boom', 'boom')];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new DisneySpec(['step' => 's', 'method' => 'GET']),
        ]);
        [$response, $err] = DisneyMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertNotNull($response->err);
    ...
    {
        $preset = new DisneyResponse(self::resp(201));
        $ctx = self::ctx(['spec' => new DisneySpec([])]);
        $ctx->out['request'] = $preset;
        [$got, $err] = DisneyMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null]);
        [, $err] = DisneyMakeFetchDef::call($ctx);
    ...
    public function test_make_fetch_def_serialises_body_and_inits_missing_result(): void
    {
        ...
        [$fetchdef, $err] = DisneyMakeFetchDef::call($ctx);
        $this->assertNull($err);
    ...
    {
        $ctx = self::ctx(['result' => new DisneyResult(['ok' => true, 'resdata' => 42])]);
        $this->assertSame(42, DisneyDone::call($ctx));
    }

    public function test_done_raises_the_error_when_not_ok(): void
    {
        $ctx = self::ctx(['result' => new DisneyResult(['ok' => false])]);
        $this->expectException(DisneyError::class);
        DisneyDone::call($ctx);
    }

    public function test_make_error_returns_resdata_when_ctrl_throw_is_false(): void
    {
        $ctx = self::ctx(['result' => new DisneyResult(['ok' => false, 'resdata' => 'fallback'])]);
        $ctx->ctrl->throw_err = false;
        $this->assertSame('fallback', DisneyMakeError::call($ctx, null));
    }
    ...
    public function test_feature_add_appends_in_call_order(): void
    {
        ...
        $a = new DisneyBaseFeature();
        $b = new DisneyBaseFeature();
        DisneyFeatureAdd::call($ctx, $a);
        DisneyFeatureAdd::call($ctx, $b);
        $this->assertSame([$a, $b], $client->features);
    }

    private static function named_feature(string $name): DisneyBaseFeature
    {
        $f = new DisneyBaseFeature();
        $f->name = $name;
        return $f;
    }
    ...
    public function test_feature_add_ordering_before_after_replace(): void
    {
        ...
        DisneyFeatureAdd::call($ctx, $miss);
    ...
    private static function auth_ctx(array $options, ?array $headers): DisneyContext
    {
        $client = new PlClient($options);
        return self::ctx([
            'client' => $client,
            'spec' => $headers === null ? null : new DisneySpec(['headers' => $headers]),
        ]);
    }
    ...
    public function test_prepare_auth_guards_a_missing_spec(): void
    {
        ...
        [, $err] = DisneyPrepareAuth::call($ctx);
    ...
    {
        $ctx = self::ctx([
            'response' => new DisneyResponse(['headers' => null]),
            'result' => new DisneyResult([]),
        ]);
        DisneyResultHeaders::call($ctx);
