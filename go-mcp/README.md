...
MCP server exposing the Disney SDK as tools, built on the
...
| Tool | Args | Returns |
|------|------|---------|
| `disney_list` | `entity`, optional `query` map | First page of records as JSON |
| `disney_load` | `entity`, `query` (e.g. `{id:N}`) | Single record as JSON |
...
After install, restart Claude Code; the `disney_list` and
`disney_load` tools become available in new sessions.
