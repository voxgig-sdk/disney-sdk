...
[MCP](https://modelcontextprotocol.io) server exposing the Disney SDK as
two agent tools — `disney_list` and `disney_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/disney-mcp)
make build

# 2. Provide credentials via the environment
export DISNEY_APIKEY=sk_live_xxx

# 3a. Install into Claude Code over stdio (most common)
claude mcp add --scope user disney \
  -- /absolute/path/to/disney-mcp -transport stdio

# 3b. …or run a shared HTTP server instead
./disney-mcp -transport http -addr :8080
```

Tool-call arguments (what an agent sends):

```jsonc
// disney_list: first page of records
{ "entity": "character" }
{ "entity": "character", "query": { } }

// disney_load: one record by id
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/disney-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user disney \
     -- "$PWD"/dist/*/disney-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `disney_list` and `disney_load` tools now appear
   in new sessions. Ask the agent to *"list character using disney"*
   and it calls `disney_list` with `{"entity":"character"}`.
...
```sh
export DISNEY_APIKEY=sk_live_xxx            # API key
export DISNEY_BASE=https://api.example.com  # optional: override the API base URL
```
...
| Tool | Args | Returns |
|------|------|---------|
| `disney_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `disney_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
| Variable | Purpose |
|----------|---------|
| `DISNEY_APIKEY` | API key sent with every request. |
| `DISNEY_BASE` | Optional override of the API base URL. |
...
```sh
./disney-mcp -transport http -addr :18080 &
...
curl -sN -X POST http://localhost:18080 \
  ...
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"disney_load","arguments":{"entity":"character","query":{"id":1}}}}'
```
